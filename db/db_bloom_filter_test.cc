...
class TestingWrappedBlockBasedFilterPolicy : public FilterPolicy {
 public:
  ...
  void CreateFilter(const MIZAR_NAMESPACE::Slice* keys, int n,
                    std::string* dst) const override {
    std::unique_ptr<MIZAR_NAMESPACE::Slice[]> user_keys(
        new MIZAR_NAMESPACE::Slice[n]);
    for (int i = 0; i < n; ++i) {
  ...
  bool KeyMayMatch(const MIZAR_NAMESPACE::Slice& key,
                   const MIZAR_NAMESPACE::Slice& filter) const override {
    counter_++;
 ...
 private:
  ...
  MIZAR_NAMESPACE::Slice convertKey(
      const MIZAR_NAMESPACE::Slice& key) const {
    return key;
  }
};
}  // namespace
...
TEST_F(DBBloomFilterTest, PrefixExtractorFullFilter) {
  BlockBasedTableOptions bbto;
  // Full Filter Block
  bbto.filter_policy.reset(MIZAR_NAMESPACE::NewBloomFilterPolicy(10, false));
  bbto.whole_key_filtering = false;
...
TEST_F(DBBloomFilterTest, PrefixExtractorBlockFilter) {
  BlockBasedTableOptions bbto;
  // Block Filter Block
  bbto.filter_policy.reset(MIZAR_NAMESPACE::NewBloomFilterPolicy(10, true));
 ...
 public:
  BloomStatsTestWithParam() {
    ...
    options_.prefix_extractor.reset(
        MIZAR_NAMESPACE::NewFixedPrefixTransform(4));
