...
void ColumnBase::Serialize(std::string* dest) const {
  MIZAR_NAMESPACE::cassandra::Serialize<int8_t>(mask_, dest);
  MIZAR_NAMESPACE::cassandra::Serialize<int8_t>(index_, dest);
}

std::shared_ptr<ColumnBase> ColumnBase::Deserialize(const char* src,
                                                    std::size_t offset) {
  int8_t mask = MIZAR_NAMESPACE::cassandra::Deserialize<int8_t>(src, offset);
...
void Column::Serialize(std::string* dest) const {
  ...
  MIZAR_NAMESPACE::cassandra::Serialize<int64_t>(timestamp_, dest);
  MIZAR_NAMESPACE::cassandra::Serialize<int32_t>(value_size_, dest);
...
std::shared_ptr<Column> Column::Deserialize(const char *src,
  ...
  int8_t mask = MIZAR_NAMESPACE::cassandra::Deserialize<int8_t>(src, offset);
  offset += sizeof(mask);
  int8_t index = MIZAR_NAMESPACE::cassandra::Deserialize<int8_t>(src, offset);
  offset += sizeof(index);
  int64_t timestamp =
      MIZAR_NAMESPACE::cassandra::Deserialize<int64_t>(src, offset);
  offset += sizeof(timestamp);
  int32_t value_size =
      MIZAR_NAMESPACE::cassandra::Deserialize<int32_t>(src, offset);
...
void ExpiringColumn::Serialize(std::string* dest) const {
  ...
  MIZAR_NAMESPACE::cassandra::Serialize<int32_t>(ttl_, dest);
}
...
std::shared_ptr<ExpiringColumn> ExpiringColumn::Deserialize(
  ...
  offset += value_size;
  int32_t ttl = MIZAR_NAMESPACE::cassandra::Deserialize<int32_t>(src, offset);
...
void Tombstone::Serialize(std::string* dest) const {
  ColumnBase::Serialize(dest);
  MIZAR_NAMESPACE::cassandra::Serialize<int32_t>(local_deletion_time_, dest);
  MIZAR_NAMESPACE::cassandra::Serialize<int64_t>(marked_for_delete_at_, dest);
}
