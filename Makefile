...
LITE ?= 0
ifeq ($(LITE), 0)
ifneq ($(filter -DMIZAR_LITE,$(OPT)),)
  # Be backward compatible and support older format where OPT=-DMIZAR_LITE is
  # specified instead of LITE=1 on the command line.
  LITE=1
endif
else ifeq ($(LITE), 1)
ifeq ($(filter -DMIZAR_LITE,$(OPT)),)
	OPT += -DMIZAR_LITE
endif
endif
...
CXXFLAGS += $(PLATFORM_SHARED_CFLAGS) -DMIZAR_DLL
CFLAGS +=  $(PLATFORM_SHARED_CFLAGS) -DMIZAR_DLL
endif
...
ifeq ($(filter -DMIZAR_ASSERT_STATUS_CHECKED,$(OPT)),)
	OPT += -DMIZAR_ASSERT_STATUS_CHECKED
endif
endif
...
ROCKSDB_PLUGIN_MKS = $(foreach plugin, $(MIZAR_PLUGINS), plugin/$(plugin)/*.mk)
include $(ROCKSDB_PLUGIN_MKS)
ROCKSDB_PLUGIN_SOURCES = $(foreach plugin, $(MIZAR_PLUGINS), $(foreach source, $($(plugin)_SOURCES), plugin/$(plugin)/$(source)))
ROCKSDB_PLUGIN_HEADERS = $(foreach plugin, $(MIZAR_PLUGINS), $(foreach header, $($(plugin)_HEADERS), plugin/$(plugin)/$(header)))
ROCKSDB_PLUGIN_PKGCONFIG_REQUIRES = $(foreach plugin, $(MIZAR_PLUGINS), $($(plugin)_PKGCONFIG_REQUIRES))
PLATFORM_LDFLAGS += $(foreach plugin, $(MIZAR_PLUGINS), $($(plugin)_LDFLAGS))
CXXFLAGS += $(foreach plugin, $(MIZAR_PLUGINS), $($(plugin)_CXXFLAGS))
...
ifneq ($(filter -DMIZAR_LITE,$(OPT)),)
	...
	# LUA is not supported under MIZAR_LITE
	LUA_PATH =
endif
...
ifdef COMPILE_WITH_UBSAN
	...
	PLATFORM_CCFLAGS += -fsanitize=undefined -fno-sanitize-recover=all -DMIZAR_UBSAN_RUN
	PLATFORM_CXXFLAGS += -fsanitize=undefined -fno-sanitize-recover=all -DMIZAR_UBSAN_RUN
endif
...
ifndef DISABLE_JEMALLOC
	ifdef JEMALLOC
		PLATFORM_CXXFLAGS += -DMIZAR_JEMALLOC -DJEMALLOC_NO_DEMANGLE
		PLATFORM_CCFLAGS  += -DMIZAR_JEMALLOC -DJEMALLOC_NO_DEMANGLE
	endif
