...
Handle<Value> DBWrapper::Open(const Arguments& args) {
  ...
  std::vector<std::string> cfs = {MIZAR_NAMESPACE::kDefaultColumnFamilyName};
  ...
  if (cfs.size() == 1) {
    db_wrapper->status_ = MIZAR_NAMESPACE::DB::Open(
        db_wrapper->options_, db_file, &db_wrapper->db_);

    return scope.Close(Boolean::New(db_wrapper->status_.ok()));
  }

  std::vector<MIZAR_NAMESPACE::ColumnFamilyDescriptor> families;

  for (std::vector<int>::size_type i = 0; i < cfs.size(); i++) {
    families.push_back(MIZAR_NAMESPACE::ColumnFamilyDescriptor(
        cfs[i], MIZAR_NAMESPACE::ColumnFamilyOptions()));
  }
...
Handle<Value> DBWrapper::Get(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Get(MIZAR_NAMESPACE::ReadOptions(), key, &value);
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ =
        db_wrapper->db_->Get(MIZAR_NAMESPACE::ReadOptions(),
...
Handle<Value> DBWrapper::Put(const Arguments& args) {
  ...
  if (args[2]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Put(MIZAR_NAMESPACE::WriteOptions(), key, value);
  } else if (db_wrapper->HasFamilyNamed(cf, db_wrapper)) {
    db_wrapper->status_ =
        db_wrapper->db_->Put(MIZAR_NAMESPACE::WriteOptions(),
                             db_wrapper->columnFamilies_[cf], key, value);
  } else {
    return scope.Close(Boolean::New(false));
  }
...
Handle<Value> DBWrapper::Delete(const Arguments& args) {
  ...
  if (args[1]->IsUndefined()) {
    db_wrapper->status_ =
        db_wrapper->db_->Delete(MIZAR_NAMESPACE::WriteOptions(), arg0);
  } else {
    if (!db_wrapper->HasFamilyNamed(arg1, db_wrapper)) {
      return scope.Close(Boolean::New(false));
    }
    db_wrapper->status_ =
        db_wrapper->db_->Delete(MIZAR_NAMESPACE::WriteOptions(),
...
Handle<Value> DBWrapper::Dump(const Arguments& args) {
  HandleScope scope;
  std::unique_ptr<MIZAR_NAMESPACE::Iterator> iterator;
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  std::string arg0      = *v8::String::Utf8Value(args[0]->ToString());

  if (args[0]->IsUndefined()) {
    iterator.reset(
        db_wrapper->db_->NewIterator(MIZAR_NAMESPACE::ReadOptions()));
  } else {
    ...
    iterator.reset(db_wrapper->db_->NewIterator(
        MIZAR_NAMESPACE::ReadOptions(), db_wrapper->columnFamilies_[arg0]));
  }
...
Handle<Value> DBWrapper::CreateColumnFamily(const Arguments& args) {
  ...
  db_wrapper->status_ = db_wrapper->db_->CreateColumnFamily(
      MIZAR_NAMESPACE::ColumnFamilyOptions(), cf_name, &cf);
...
bool DBWrapper::AddToBatch(MIZAR_NAMESPACE::WriteBatch& batch, bool del,
                           Handle<Array> array) {
...
Handle<Value> DBWrapper::WriteBatch(const Arguments& args) {
  ...
  MIZAR_NAMESPACE::WriteBatch batch;
  bool well_formed;
  ...
  db_wrapper->status_ =
      db_wrapper->db_->Write(MIZAR_NAMESPACE::WriteOptions(), &batch);
...
Handle<Value> DBWrapper::CompactRangeDefault(const Arguments& args) {
  ...
  DBWrapper* db_wrapper = ObjectWrap::Unwrap<DBWrapper>(args.This());
  MIZAR_NAMESPACE::Slice begin = *v8::String::Utf8Value(args[0]->ToString());
  MIZAR_NAMESPACE::Slice end = *v8::String::Utf8Value(args[1]->ToString());
