...
protobuf_mutator::libfuzzer::PostProcessorRegistration<DBOperations> reg = {
    [](DBOperations* input, unsigned int /* seed */) {
      const MIZAR_NAMESPACE::Comparator* comparator =
...
DEFINE_PROTO_FUZZER(DBOperations& input) {
  ...
  auto fs = MIZAR_NAMESPACE::FileSystem::Default();
  if (fs->FileExists(kDbPath, MIZAR_NAMESPACE::IOOptions(), /*dbg=*/nullptr)
          .ok()) {
  ...
  CHECK_OK(MIZAR_NAMESPACE::DB::Open(options, kDbPath, &db));

  for (const DBOperation& op : input.operations()) {
    switch (op.type()) {
      case OpType::PUT: {
        CHECK_OK(
            db->Put(MIZAR_NAMESPACE::WriteOptions(), op.key(), op.value()));
        kv[op.key()] = op.value();
        break;
      }
      case OpType::MERGE: {
      ...
      case OpType::DELETE: {
        CHECK_OK(db->Delete(MIZAR_NAMESPACE::WriteOptions(), op.key()));
        kv.erase(op.key());
        break;
      }
      case OpType::DELETE_RANGE: {
        // [op.key(), op.value()) corresponds to [begin, end).
        CHECK_OK(db->DeleteRange(MIZAR_NAMESPACE::WriteOptions(),
  ...
  CHECK_OK(db->Close());
  delete db;
  CHECK_OK(MIZAR_NAMESPACE::DestroyDB(kDbPath, options));
}
