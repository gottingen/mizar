...
jlong Java_org_rocksdb_PlainTableConfig_newTableFactoryHandle(
  ...
  MIZAR_NAMESPACE::PlainTableOptions options =
      MIZAR_NAMESPACE::PlainTableOptions();
  ...
  options.encoding_type =
      static_cast<MIZAR_NAMESPACE::EncodingType>(jencoding_type);
...
jlong Java_org_rocksdb_BlockBasedTableConfig_newTableFactoryHandle(
  ...
  MIZAR_NAMESPACE::BlockBasedTableOptions options;
  ...
  options.index_type =
      MIZAR_NAMESPACE::IndexTypeJni::toCppIndexType(jindex_type_value);
  options.data_block_index_type =
      MIZAR_NAMESPACE::DataBlockIndexTypeJni::toCppDataBlockIndexType(
          jdata_block_index_type_value);
  options.data_block_hash_table_util_ratio =
      static_cast<double>(jdata_block_hash_table_util_ratio);
  options.checksum = MIZAR_NAMESPACE::ChecksumTypeJni::toCppChecksumType(
  ...
  if (options.no_block_cache) {
    ...
    if (jblock_cache_handle > 0) {
      std::shared_ptr<MIZAR_NAMESPACE::Cache> *pCache =
      ...
      if (jblock_cache_num_shard_bits > 0) {
        options.block_cache = MIZAR_NAMESPACE::NewLRUCache(
  ...
  if (jpersistent_cache_handle > 0) {
    std::shared_ptr<MIZAR_NAMESPACE::PersistentCache> *pCache =
        reinterpret_cast<std::shared_ptr<MIZAR_NAMESPACE::PersistentCache> *>(
  ...
  if (jblock_cache_compressed_handle > 0) {
    ...
    if (jblock_cache_compressed_num_shard_bits > 0) {
      options.block_cache_compressed = MIZAR_NAMESPACE::NewLRUCache(
  ...
  if (jfilter_policy_handle > 0) {
    std::shared_ptr<MIZAR_NAMESPACE::FilterPolicy> *pFilterPolicy =
        reinterpret_cast<std::shared_ptr<MIZAR_NAMESPACE::FilterPolicy> *>(
  ...
  options.index_shortening =
      MIZAR_NAMESPACE::IndexShorteningModeJni::toCppIndexShorteningMode(
          jindex_shortening);

  return reinterpret_cast<jlong>(
      MIZAR_NAMESPACE::NewBlockBasedTableFactory(options));
}
