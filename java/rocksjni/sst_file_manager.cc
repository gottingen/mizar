...
jlong Java_org_rocksdb_SstFileManager_newSstFileManager(
  ...
  auto* env = reinterpret_cast<MIZAR_NAMESPACE::Env*>(jenv_handle);
  MIZAR_NAMESPACE::Status s;
  MIZAR_NAMESPACE::SstFileManager* sst_file_manager = nullptr;

  if (jlogger_handle != 0) {
    auto* sptr_logger =
        reinterpret_cast<std::shared_ptr<MIZAR_NAMESPACE::Logger>*>(
            jlogger_handle);
    sst_file_manager = MIZAR_NAMESPACE::NewSstFileManager(
  ...
  if (!s.ok()) {
    ...
    MIZAR_NAMESPACE::RocksDBExceptionJni::ThrowNew(jnienv, s);
  }
  auto* sptr_sst_file_manager =
      new std::shared_ptr<MIZAR_NAMESPACE::SstFileManager>(sst_file_manager);
...
jobject Java_org_rocksdb_SstFileManager_getTrackedFiles(JNIEnv* env,
  ...
  // MIZAR_NAMESPACE::HashMapJni::fromCppMap(env, tracked_files);

  const jobject jtracked_files = MIZAR_NAMESPACE::HashMapJni::construct(
  ...
  const MIZAR_NAMESPACE::HashMapJni::FnMapKV<const std::string,
  ...
  if (!MIZAR_NAMESPACE::HashMapJni::putAll(env, jtracked_files,
                                             tracked_files.begin(),
